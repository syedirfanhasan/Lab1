...
unsigned long ST7735_OutString(unsigned short x, unsigned short y, unsigned char *pt, short textColor, unsigned int arrayLength){
  unsigned long count = 0;
  unsigned int j = 1;
  if(y>15) return 0;
  while((*pt) && (j<=arrayLength)){
...
void ST7735_OutUDec(unsigned long n){
  ...
  ST7735_OutString(Cursor,StLine,Message,ST7735_YELLOW,12);
}
...
void ST7735_OutUDec2(unsigned long n, unsigned long l){
  ...
  Message[Messageindex] = 0; // terminate
  ST7735_OutString(14,l,Message,ST7735_YELLOW,12);
}
...
long Ymax,Ymin,Xmax,Xmin,X;        // X goes from 0 to 127
long Yrange,Xrange; //YrangeDiv2;
...
// Outputs: none
void ST7735_PlotClear(long xmin, long xmax, long ymin, long ymax){
...
// Outputs: none
void ST7735_PlotPoint(long x, long y){
  long i,j;
  ...
  //Convert the y value to the corresponding vertical pixel.
  if(y<Ymin)
  {
	  y=Ymin;
  }
  ...
  //Convert the x value to the corresponding horizontal pixel.
  if(x<Xmin)
  {
	  x=Xmin;
  }
